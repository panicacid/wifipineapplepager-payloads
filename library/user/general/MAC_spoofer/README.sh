...
**1. Workflow Briefing**
The payload begins with a brief overview of the process and a safety warning regarding network disconnection.

![Startup](screens/Capture01.PNG)

**2. Select Interface**
The tool scans for all available network interfaces, prioritizing the most useful ones at the top of the list.
* **Action:** Enter the ID of the interface you wish to spoof.
...
Choose the category that matches your target environment.
* **0. Restore Original:** Reverts the interface to its factory hardware address.
* **1-5. Environments:** Choose from Home, Corporate, Commercial, Industrial, or Ethernet profiles.
...
Select the specific device to emulate. The script generates a valid MAC address using that vendor's OUI and a randomized suffix.

![Profile List](screens/Capture06.PNG)
![Profile Selection](screens/Capture07.PNG)

**5. Confirm Action**
Review your selection. The screen displays the target Interface, the new Hostname, and the new MAC address for confirmation.

![Confirm Spoof](screens/Capture08.PNG)

**6. Apply & Verify**
The tool temporarily disables the network interface, applies the new identity, and re-enables it. It then verifies the active state against the kernel.
